...
func TestSQLiteSessionTodoStatusAndDiff(t *testing.T) {
	...
	if err := store.SetStatus(ctx, info.ID, session.StatusInfo{Type: "retry", Attempt: 2, Message: "again", Next: 123}); err != nil {
		t.Fatalf("SetStatus() error = %v", err)
	}
	status, err := store.Status(ctx, info.ID)
	if err != nil {
		t.Fatalf("Status() error = %v", err)
	}
	if status.Type != "retry" || status.Attempt != 2 || status.Next != 123 {
